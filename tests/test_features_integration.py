...
def test_robust_gap_features_multi_input(dummy_data):
    ...
    assert (eda_tmp_data.index[1:] - eda_tmp_data.index[:-1]).max() == pd.Timedelta("25 s")

    res_df = feature_collection.calculate(eda_tmp_data, return_df=True, approve_sparsity=True)
    assert res_df.isna()[1:4].all().all()
    assert res_df[4:].isna().any().any() == False
