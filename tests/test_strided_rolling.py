...
def test_sequence_stroll_indexing():
    ...
    ## Force
    sr = SequenceStridedRolling(
        s, window=3, strides=[1], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0, 1, 2])
    sr = SequenceStridedRolling(
        s, window=3, strides=[2], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0, 2])
    sr = SequenceStridedRolling(
        s, window=3, strides=[4], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0, 4])
    sr = SequenceStridedRolling(
        s, window=3, strides=[5], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])
    sr = SequenceStridedRolling(
        s, window=3, strides=[50], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])

    sr = SequenceStridedRolling(
        s, window=4, strides=[1], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0, 1])

    sr = SequenceStridedRolling(
        s, window=5, strides=[1], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])
    sr = SequenceStridedRolling(
        s, window=5, strides=[2], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])
    sr = SequenceStridedRolling(
        s, window=6, strides=[1], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])
    sr = SequenceStridedRolling(
        s, window=6, strides=[2], window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0])
...
def test_time_stroll_indexing():
    ...
    sr = TimeStridedRolling(
        s,
        window=pd.Timedelta(3, unit="h"),
        strides=[pd.Timedelta(1, unit="h")],
        window_idx="begin",
    )
    ...
    sr = TimeStridedRolling(
        s,
        window=pd.Timedelta(3, unit="h"),
        strides=[pd.Timedelta(50, unit="h")],
        window_idx="begin",
    )
    assert np.all(sr.index == get_time_index([0]))
...
def test_time_index_sequence_stroll_indexing():
    ...
    ## Force
    sr = TimeIndexSampleStridedRolling(
        s, window=3, strides=1, window_idx="begin", include_final_window=True
    )
    assert np.all(sr.index == [0, 1, 2])
    sr = TimeIndexSampleStridedRolling(
    ...
    assert np.all(sr.index == [0, 2])
    sr = TimeIndexSampleStridedRolling(
        s, window=3, strides=3, window_idx="begin", include_final_window=True
    )
...
def test_time_stroll_indexing_multiple_strides():
    ...
    sr = TimeStridedRolling(
        s,
        window=pd.Timedelta(3, unit="h"),
        strides=[pd.Timedelta(3, unit="h"), pd.Timedelta(5, unit="h")],
        window_idx="begin",
    )
...
def test_sequence_stroll_indexing_segment_start_idxs():
    ...
    ## No Force
    sr = SequenceStridedRolling(
        s, window=3, segment_start_idxs=segment_start_idxs, window_idx="begin"
    )
...
def test_sequence_stroll_indexing_segment_end_idxs():
    ...
    ## No Force
    sr = SequenceStridedRolling(
        s, window=3, segment_end_idxs=segment_end_idxs, window_idx="end"
    )
    assert sr.strides is None
    assert np.all(sr.index == segment_end_idxs)

    sr = SequenceStridedRolling(
        s, window=3, strides=[3, 5], segment_end_idxs=segment_end_idxs, window_idx="end"
    )
    assert sr.strides is None
    assert np.all(sr.index == segment_end_idxs)
...
def test_time_stroll_indexing_segment_start_idxs():
    ...
    assert np.all(
        sr.index
        == [t + pd.Timedelta(3, unit="h") for t in get_time_index([0, 5, 7, 10])]
    )
...
def test_time_stroll_indexing_segment_start_idxs_outside_valid_range():
    ...
    segment_start_idxs = s.index[[0, 5, 7, 10]].values
    segment_start_idxs = np.append(
        segment_start_idxs, (s.index[[0]] + pd.Timedelta(200, unit="h")).values
    )
    ...
    assert sr.strides is None
    assert np.all(
        sr.index == [t + pd.Timedelta(3, unit="h") for t in segment_start_idxs]
    )
...
def test_time_stroll_indexing_segment_end_idxs_outside_valid_range():
    ...
    segment_end_idxs = s.index[[0, 5, 7, 10]].values
    segment_end_idxs = np.append(
        segment_end_idxs, (s.index[[0]] + pd.Timedelta(200, unit="h")).values
    )
...
def test_sequence_stroll_apply_func_vectorized_segment_idxs():
    ...
    ## No Force
    sr = SequenceStridedRolling(
        s, window=3, segment_start_idxs=segment_idxs, window_idx="begin"
    )
    assert_1col_df_equal(sr.apply_func(f), sr.apply_func(f_vect))
    ...
    sr = SequenceStridedRolling(
        s,
        window=3,
        segment_start_idxs=segment_idxs,
        window_idx="begin",
        include_final_window=True,
    )
    assert_1col_df_equal(sr.apply_func(f), sr.apply_func(f_vect))
    ...
    ## No Force
    sr = SequenceStridedRolling(
        s, window=3, segment_end_idxs=segment_idxs, window_idx="begin"
    )
    assert_1col_df_equal(sr.apply_func(f), sr.apply_func(f_vect))
    ...
    with pytest.raises(Exception):
        ...
        sr = SequenceStridedRolling(
            s, window=3, segment_end_idxs=segment_end_idxs, window_idx="begin"
        )
        sr.apply_func(f_vect)
...
def test_sequence_stroll_apply_func_vectorized_multi_output():
    ...
    f_vect = FuncWrapper(
        min_max, output_names=["min_vect", "max_vect"], vectorized=True, axis=-1
    )
    ...
    def assert_2col_df_equal(s1, s2):
        ...
        for name in ["min", "max"]:
            assert np.all(
                s1.filter(like=name).values.ravel()
                == s2.filter(like=name).values.ravel()
            )
